...
with h5py.File('meas.hdf5', 'a') as f:
    ...
    # existing data
    try:
        rme.save_object(f, meas.name, meas)
    except ValueError:
        del f[meas.name]
        rme.save_object(f, meas.name, meas)
    print(f[meas.name])
...
with h5py.File('meas.hdf5', 'r') as f:
    meas = rme.load_object(f[meas.name], load_big_objects=True)
    print(meas)
...
m1 = rme.RMEMeas.from_nom('mymeas', xr.DataArray(np.zeros((2, 2))))
...
m2 = rme.RMEMeas.from_xml('mymeas.meas', from_csv=from_txt)
