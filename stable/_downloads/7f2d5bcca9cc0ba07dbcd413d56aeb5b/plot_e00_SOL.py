...
with h5py.File(defs_path, 'r') as f:
    def_short = rme.load_object(f['Short']).nom
    def_open = rme.load_object(f['Open']).nom
    def_load = rme.load_object(f['Load']).nom
...
myprop = rme.RMEProp(sensitivity=True)
...
raw_dut = rme.RMEMeas.from_nom('DUT', raw_dut)
raw_short = rme.RMEMeas.from_nom('DUT', raw_short)
raw_open = rme.RMEMeas.from_nom('DUT', raw_open)
raw_load = rme.RMEMeas.from_nom('DUT', raw_load)

# %%
# Lets also grab our definitions with the full uncertainty information
with h5py.File(defs_path, 'r') as f:
    def_short = rme.load_object(f['Short'])
    def_open = rme.load_object(f['Open'])
    def_load = rme.load_object(f['Load'])
